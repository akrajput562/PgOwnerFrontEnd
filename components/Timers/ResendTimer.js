...
const ResendTimer = ({activeResend, setActiveResend, targetTimeInSeconds,resendEmail,resendStatus, ...props}) => {
    ...
    <StyledView {...props}>
        ...
        <PressableText onPress={() => resendEmail(triggerTimer)} disabled={!activeResend} 
        ...
        {!activeResend && (
        <SmallText>In <SmallText style={{fontWeight:'bold' }}>  {timeLeft || targetTime}</SmallText> second(s)</SmallText>
    )}
        </StyledView>
    );
};
